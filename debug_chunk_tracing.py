"""Enhanced debug script with monkey-patching to trace chunk processing."""
...
original_lookahead = two_phase._find_best_chunk_selection_with_lookahead
call_count = [0]

def traced_lookahead(chunk_prices, next_chunk_prices, low_price_threshold, target,
                     min_consecutive_periods, max_gap_between_periods, max_gap_from_start, aggressive):
    call_count[0] += 1
    print(f"[Call #{call_count[0]}] _find_best_chunk_selection_with_lookahead:")
    print(f"  chunk_len={len(chunk_prices)}, target={target}, min_consecutive={min_consecutive_periods}")
    print(f"  max_gap_from_start={max_gap_from_start}, max_gap_between={max_gap_between_periods}")
    
    result = original_lookahead(chunk_prices, next_chunk_prices, low_price_threshold, target,
                                 min_consecutive_periods, max_gap_between_periods, max_gap_from_start, aggressive)
    print(f"  -> returned {len(result)} selections: {result[:20]}")
    print()
    return result

two_phase._find_best_chunk_selection_with_lookahead = traced_lookahead
